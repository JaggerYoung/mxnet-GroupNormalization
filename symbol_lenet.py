...
def get_symbol(num_classes=10, add_stn=False, **kwargs):
    ...
    # GN
    gamma = mx.symbol.Variable(name='gamma', shape=(50,), init=mx.init.One())
    beta = mx.symbol.Variable(name='beta', shape=(50,), init=mx.init.Zero())
    loc = mx.symbol.Custom(data=pool2, gamma=gamma, beta=beta, op_type='GroupNormalization', num_groups=2)
    # first fullc
